	...
	switch (m_byAlign)
	{
		...
		case ST_ALIGN_HORIZ:
			...
			else
			{
				...
				if(ST_ALIGN_IMAGE_LEFT ==m_byAlign )
				{
                   rpImage->left = 1+rBtn.left; 
				   rpTitle->left = 12+rBtn.left;
				//   rpTitle->right = rBtn.right-1;
				 //  	rpImage->top += ((rpImage->Height() - (long)dwHeight)/2)+10;
				}
				else
				{
		...
		case ST_ALIGN_HORIZ_RIGHT:
			...
			else
			{
				...
				if(m_byAlign == ST_ALIGN_IMAGE_RIGHT)
				{
					rpTitle->right = rpTitle->Width() -13;// dwWidth - m_ptImageOrg.x;
					rpTitle->left = 1;//m_ptImageOrg.x;
					rpImage->left = rBtn.right-12;//dwWidth;//+rpImage->Width();// - m_ptImageOrg.x;
				}
				else
				{
...
DWORD CButtonST::SetURL(LPCTSTR lpszURL)
{
	...
	{
		// Store the URL
		//wcsncpy_s(m_szURL,_MAX_PATH, lpszURL, _MAX_PATH);//(wchar_t*) modify by lee.2014-6-30
	} // if
