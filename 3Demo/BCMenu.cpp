...
void BCMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
		int nLength = sizeof(wchar_t)*(wcslen(szWideString)+1);
		m_szMenuText = new wchar_t[nLength];
		if (m_szMenuText)
			wcsncpy_s(m_szMenuText,nLength,szWideString,_TRUNCATE);
    }
	else
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	do{
		...
		nLen = 0;
		int nLength = wcslen((wchar_t *)pTp)+1;
		szCaption=new wchar_t[nLength];
		wcsncpy_s(szCaption,nLength,(wchar_t *)pTp,_TRUNCATE);
